...
export function initSlideDeck(root) {
  ...
  const evaluateActiveTwoCol = () => {
    ...
    const wasTwo = s.classList.contains('two-col');
    try {
      if (wasTwo) s.classList.remove('two-col');
      void s.offsetHeight; // force reflow before measuring
    } catch (_) {}
    const needsTwo = s.scrollHeight > s.clientHeight + 8;
    if (needsTwo) s.classList.add('two-col');
    else s.classList.remove('two-col');
  };
