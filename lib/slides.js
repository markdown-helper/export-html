 ...
 *
 * Features:
 * - detectSlidesEnabled(mdText): Enable slides when front matter contains marp/slides/mheSlides true,
 *   or when top-level horizontal rule separators ('---' on their own line) exist outside code fences
 *   and outside front matter.
 * - splitMarkdownSlides(mdText): Split markdown into slides on '---' separators outside code fences,
 *   preserving YAML front matter on the first slide.
 ...
 * - Enabled if front matter flags marp/slides/mheSlides are true.
 * - Otherwise enabled if there is at least one top-level '---' separator outside code fences,
 *   ignoring the initial front matter fence.
 * @param {string} md
 * @returns {boolean}
 */
export function detectSlidesEnabled(md) {
  try {
    const { content, flags } = extractFrontMatter(md);
    if (flags.marp || flags.slides || flags.mheSlides) return true;

    let text = content;
    // Temporarily remove fenced code blocks to avoid false positives
    const blocks = [];
    text = text.replace(/```[\s\S]*?```/g, (m) => {
      blocks.push(m);
      return `%%SL_BLOCK_${blocks.length - 1}%%`;
    });
    // Check for a line containing only '---' (optional surrounding whitespace)
    const hasSep = /^(\s*)---(\s*)$/m.test(text);
    return hasSep;
  } catch (_) {
    return false;
  }
...
export function splitMarkdownSlides(md) {
  ...
  // Attach front matter back to the first slide to keep downstream processors aware
  if (frontMatter && slides.length) {
    slides[0] = `${frontMatter}${slides[0]}`;
  }
