  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        // Allow optional leading whitespace, list marker (- or *), or blockquote (>)
        // Example matches:
        //   [^1]: text
        //     [^id]: indented
        //   - [^id]: inside list
        //   * [^id]: inside list
        //   > [^id]: inside blockquote
  ...
  try {
    if (Array.isArray(footnoteOrder) && footnoteOrder.length) {
      ...
      footnoteOrder.forEach((key) => {
        ...
        li.id = `fn-${key}`;
        let text = (footnoteDefs && footnoteDefs[key])
          ? footnoteDefs[key]
          : ((typeof window !== 'undefined' && window.__mheFootnoteDefs && window.__mheFootnoteDefs[key])
              ? window.__mheFootnoteDefs[key]
              : null);

        // Fallback: parse single-line definition directly from original markdown if available
